...
M = 50; % doing this for M 'training' images to debug program.
S=[];
% figure(1);
for i=1:M
...
% um=mean(mean(double(S),2));
% ustd =mean(std(double(S),0,1));
S=double(S);
max_im=max(S);
min_im=min(S);
new_max=255;
...
% mean_img=reshape(uint8(psi_mat),icol,irow);
% % mean_img=mean_img';
% figure(3);
% subplot(1,1,1);
...
V_mat=fliplr(V_mat);% now, top 'xx' eigenvectors are arranged left to right in V_mat

%Based upon the user specified threshold for 
Mprime=min(find(cumVar>=threshold));% this is the no of eigenvectors/eigenfaces that need to be considered
                             % in order to explain at least 95% of the
                             % variance in the whale image data!
%Zeroing out the remaining eigenvectors as they are no longer needed
V_mat(:,Mprime+1:M)=0;% in effect, the no of eigenfaces that are computed will be Mprime, not M.
...
%     img=reshape(U_EigFace(:,i),icol,irow);
%     % img=img';
...
toc
Whale_Eigenface_V2.m
Displaying Whale_Eigenface_V2.m.